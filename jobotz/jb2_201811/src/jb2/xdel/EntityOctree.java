...
public class EntityOctree {
    ...
    protected int[] objects = new int[defaultNodeCount];
    
    protected float xmin, xmax, ymin, ymax, zmin, zmax;
        
    public EntityOctree(float xmin, float xmax, float ymin, float ymax, float zmin, float zmax) {
        this.xmin = xmin;
        this.xmax = xmax;
        this.ymin = ymin;
        this.ymax = ymax;
    ...
    protected void getIntersecting(int node, BoundingBox bb, IntList store) {
        ...
        while(checknodes.size>0) {
            ...
            if(node<0) {
                ...
                int leaf = -node;
                int leafpos = leaf*3;
                if(bb.contains(leafCoord[leafpos], leafCoord[leafpos+1], leafCoord[leafpos+2])) {
            ...
            if(!bb.intersects(nodeBounds, nodepos)) {
                // no intersection
                continue;
            }

            if(bb.contains(nodeBounds, nodepos)) {
